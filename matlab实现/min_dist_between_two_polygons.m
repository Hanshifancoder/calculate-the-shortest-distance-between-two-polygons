...
%   P1 & P2 are structure containing two fields: x & y
%   Display_solution is binary variable that enables or not the plot of the
%   solution
%
% The function starts by checking polygons are intersecting. In this case,
% the minimum distance is 0.
% Otherwise, the distance between all vertices and edges of the two
% polygons are computed. Further details of the implementations can be
% found in the code.
% 
% 2008 12 15
% Guillaume Jacquenot
...
% A loop is used to sweep along segments of P1.
 dP1x = diff(P1.x);  % 上面最后补充一个点的目的是为了构造这条有效边
 dP1y = diff(P1.y);
sdP12 = dP1x.^2 + dP1y.^2;  % 获得多边形每条边的长度
if any(sdP12 == 0)  % 判断是否有重叠的点出现
    warning('min_dist_between_two_polygons:w1',...
        'Two successive points of P1 are indentical.')
end
K1 = zeros(Npts_P1-1,Npts_P2);
D1 = zeros(Npts_P1-1,Npts_P2);  % D1中保存的是P2中的所有顶点到P1中所有边的距离.
for i = 1:numel(P1.x)-1  % 遍历多边形上的每个点,遍历多边形上面的每条边
    % Compute the closest distance between a point and a segment
    % Computation of the parameter k  计算直线参数化表示中的参数k
    k = -(dP1x(i) * (P1.x(i) - P2.x) +...
          dP1y(i) * (P1.y(i) - P2.y)) / sdP12(i);  % 这里计算的是P2上面的点到P1每条边之间的距离
    % 接下来分三种情况进行讨论
    I1 = k < 0;  % 需要特别注意的是这里的k是一个向量所以I1也是一个向量，而且长度为Npts_P2 
    I2 = k > 1;
    I  =~(I1|I2);  % 即0小于等于k小于等于1
    % Computation of the minimum squared distances between all points of 
    % P2 and the i-th segment of P1.
    D1(i,I1) = (P2.x(I1)- P1.x(i  )).^2+(P2.y(I1)-P1.y(i  )).^2;  % D1中的每一行表示P1中的一段线段到P2所有点之间的距离，这里表示的是哪里为真就往哪里赋值，不为真的地方不需要赋值
...
[min_d1,ind_seg1] = min(D1);  % 返回矩阵中每列的最小元素和其对应的索引
